...
- (void)resolveDependencies {
    ...
    [self.proxies enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key,
                                                      ...
                                                      BOOL * _Nonnull stop) {
        if (proxy.target != nil && proxy.status == WPLazyProxyStatusShouldResolve) {
    ...
    [self.proxies enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key,
                                                      WPLazyProxy<WPInjectionComponent> * _Nonnull proxy,
                                                      BOOL * _Nonnull stop) {
        if (proxy.target != nil && proxy.status != WPLazyProxyStatusDidResolved) {
            [proxy instanceDidFinishInjectDependencies];
            proxy.status = WPLazyProxyStatusDidResolved;
        }
    }];
